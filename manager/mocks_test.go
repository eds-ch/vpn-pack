...
type mockFirewallService struct {
	...
	auditAndFixTsInputOrderFn       func(ctx context.Context) error
...
func (m *mockFirewallService) AuditAndFixTsInputOrder(ctx context.Context) error {
	if m.auditAndFixTsInputOrderFn != nil {
		return m.auditAndFixTsInputOrderFn(ctx)
	}
	return nil
}
...
func (m *mockFirewallService) DiscoverChainPrefix(ctx context.Context, zoneID string) string {
	if m.discoverChainPrefixFn != nil {
		return m.discoverChainPrefixFn(ctx, zoneID)
	}
	return ""
}
